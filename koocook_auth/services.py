...
class BasePreferenceManager:
    DEFINED_PREFERENCES: tuple = tuple()
    ...
    def register_default_preferences(self):
        for section in self.DEFINED_PREFERENCES:
            for pref in section:
    ...
    def all(self) -> list:
        ...
        for pref_name in self.preferences:
            pref = self.to_preference(pref_name, self.preferences[pref_name])
            prefs.append(pref)
        return prefs
    ...
    def get_full(self, name: str):
        return self.to_preference(name, self.preferences[name])

    @classmethod
    def to_preference(cls, key: str, value: str = ''):
        try:
            section = get_section(key, cls.DEFINED_PREFERENCES)
        except ValueError:
            return None
...
def get_section(pref_key, table) -> Preference:
